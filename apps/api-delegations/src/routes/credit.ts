...
const getCollectionQuery = z.object({
  account: z.string().refine((val) => val.length > 0, {
    message: 'invalid type',
  }).optional(),
  accounts: z.string().refine((val) => val.length > 0, {
    message: 'invalid type',
  }).optional(),
});

const creditRouter = new Hono()
  .get('/', zValidator('param', getCollectionParm), zValidator('query', getCollectionQuery), async (c) => {
