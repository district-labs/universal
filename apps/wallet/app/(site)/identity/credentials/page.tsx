...
export default function IdentityCredentialsPage() {
  ...
  const did = useMemo(() => address ? constructDidIdentifier({
  ...
  const missingCredentialOptions = useMemo(() => {
    ...
    return CREDENTIAL_OPTIONS.filter(credential => !credentialsQuery.data?.some(claimedCredential => claimedCredential.type === credential.type))
    ...
    <>
      ...
      <section className="py-8">
        <div className='mx-auto grid max-w-screen-xl grid-cols-1 gap-x-5 lg:grid-cols-3'>
          {
            credentialsQuery.isLoading && Array.from({ length: 3 }).map((_, i) =>
        ...
        <h2 className='px-10 mb-4 mt-6 text-2xl font-bold'>Created Credentials</h2>
        <div className='mx-auto grid max-w-screen-xl grid-cols-1 gap-x-5 lg:grid-cols-3'>
          {
            credentialsQuery.isLoading && Array.from({ length: 3 }).map((_, i) =>
              <Skeleton key={String(i)} className='w-full h-72' />
          ...
          }
          {
            ...
            credentialsQuery.data?.map((credential) => (
              <Card key={credential.id}>
                <CardHeader className="pb-2 text-lg capitalize font-bold">{credential.type}</CardHeader>
                <CardContent>
